...
//merge the point x and point y.  meanwhile merge the trees in which they stay.
void MFSET::merge(int x, int y) {
  int rx = find(x);   //find the root for the point x
  int ry = find(y);   //find the root for the point y
  
  if( rx != ry ) {
    nSets--;
    if (mf[rx] == mf[ry]) { //height of the tree rooting at rx == height(ry)
      mf[ry] = rx;
      mf[rx]--; //Increase height(rx)
    }
    else { //Different height
      if( mf[rx] < mf[ry] ) // height(rx) > height(ry)
        	mf[ry] = rx;
      else
        	mf[rx] = ry;
    }
